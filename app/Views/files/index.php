...
<div class="wrap">
  ...
  <form class="filters panel" id="filterForm" method="get" action="<?= base_url('files') ?>">
    ...
    <div><label for="filterFolder">Folder</label><select id="filterFolder" name="folder"><option value="">All</option><?php foreach ($folders as $folder): ?><option value="<?= esc($folder, 'attr') ?>" <?= $filters['folder'] === $folder ? 'selected' : '' ?>><?= esc($folder) ?></option><?php endforeach; ?></select></div>
    ...
    <aside class="panel">
      ...
      <form id="uploadForm">
        ...
        <div class="dropzone" id="dropzone">
          <p>Drop files here, or choose files/folder</p>
          <div class="upload-pickers">
            <label class="picker-button" for="fileInput">&#128196; Choose files</label>
            <label class="picker-button folder" for="folderInput">&#128193; Choose folder</label>
          </div>
          <input class="upload-input-hidden" type="file" id="fileInput" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.zip,.png,.jpg,.jpeg,.webp,.gif,.heic">
          <input class="upload-input-hidden" type="file" id="folderInput" multiple webkitdirectory directory accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.zip,.png,.jpg,.jpeg,.webp,.gif,.heic">
          <div class="selection-summary" id="selectionSummary">Nothing selected</div>
        </div>
        <p class="file-hint">Maximum <?= (int) $maxMb ?> MB per file. Folder uploads keep each file's relative folder path. Empty folders cannot be uploaded by browsers.</p>
...
const ALLOWED_EXTENSIONS = new Set(['pdf','doc','docx','xls','xlsx','ppt','pptx','txt','csv','zip','png','jpg','jpeg','webp','gif','heic']);
...
function extensionOf(name){const part=String(name).split('.').pop();return part&&part!==name?part.toLowerCase():'';}
function relativePath(file){return String(file.webkitRelativePath||file.name).replace(/\\/g,'/').replace(/^\/+|\/+$/g,'');}
function folderPath(file){const path=relativePath(file);const cut=path.lastIndexOf('/');return cut>0?path.slice(0,cut):'';}
function chooseUploadFiles(fileList,source='files'){
  const incoming=[...fileList]; skippedSelectionCount=0;
  selectedUploadFiles=incoming.filter(file=>{const valid=ALLOWED_EXTENSIONS.has(extensionOf(file.name))&&file.size>0;if(!valid)skippedSelectionCount++;return valid;});
  fileInput.value='';folderInput.value='';refreshSelected(source);
}
function refreshSelected(source='files'){
  selectedFiles.innerHTML='';
  const previewLimit=60;
  selectedUploadFiles.slice(0,previewLimit).forEach(file=>{const row=document.createElement('div');row.className='selected-file';row.innerHTML='<span class="selected-file-info"><span class="selected-file-name"></span><span class="selected-file-path"></span></span><strong></strong>';row.querySelector('.selected-file-name').textContent=file.name;const path=folderPath(file);const pathNode=row.querySelector('.selected-file-path');pathNode.textContent=path?path:'Vault root';row.querySelector('strong').textContent=formatBytes(file.size);selectedFiles.appendChild(row);});
  if(selectedUploadFiles.length>previewLimit){const more=document.createElement('div');more.className='file-hint';more.textContent='+'+(selectedUploadFiles.length-previewLimit)+' more files selected';selectedFiles.appendChild(more);}
  const rootFolders=new Set(selectedUploadFiles.map(folderPath).filter(Boolean).map(path=>path.split('/')[0]));
  let text=selectedUploadFiles.length+' valid file'+(selectedUploadFiles.length===1?'':'s')+' selected';
  if(source==='folder'&&rootFolders.size)text+=' from '+[...rootFolders].join(', ');
  if(skippedSelectionCount)text+=' · '+skippedSelectionCount+' unsupported or empty skipped';
  selectionSummary.textContent=selectedUploadFiles.length?text:'Nothing selected';
}
fileInput.addEventListener('change',()=>chooseUploadFiles(fileInput.files,'files'));
folderInput.addEventListener('change',()=>chooseUploadFiles(folderInput.files,'folder'));
...
uploadForm.addEventListener('submit',async e=>{
  e.preventDefault(); const files=[...selectedUploadFiles]; if(!files.length){setStatus('Choose at least one file.',true);return;}
  ...
  try{
    ...
    progressBar.style.width='100%';setStatus(completed+' file'+(completed===1?'':'s')+' added successfully.');uploadForm.reset();selectedUploadFiles=[];skippedSelectionCount=0;selectedFiles.innerHTML='';selectionSummary.textContent='Nothing selected';uploadBtn.textContent='Add to vault';
  }catch(err){
