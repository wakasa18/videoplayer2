...
const shareModal=document.getElementById('shareModal'),shareForm=document.getElementById('shareForm'),shareModalTitle=document.getElementById('shareModalTitle'),shareHelp=document.getElementById('shareHelp'),shareDuration=document.getElementById('shareDuration'),shareMaxDownloads=document.getElementById('shareMaxDownloads'),sharePageTitle=document.getElementById('sharePageTitle'),shareDisplayName=document.getElementById('shareDisplayName'),shareMessage=document.getElementById('shareMessage'),shareNotifyFirstOpen=document.getElementById('shareNotifyFirstOpen'),shareNotifyLimit=document.getElementById('shareNotifyLimit'),shareNotifyExpiring=document.getElementById('shareNotifyExpiring'),shareCreateBtn=document.getElementById('shareCreateBtn'),shareResult=document.getElementById('shareResult'),shareLinkInput=document.getElementById('shareLinkInput'),shareCopyBtn=document.getElementById('shareCopyBtn'),shareQrBtn=document.getElementById('shareQrBtn'),shareResultNote=document.getElementById('shareResultNote'),shareList=document.getElementById('shareList'),qrModal=document.getElementById('qrModal'),qrCodeCanvas=document.getElementById('qrCodeCanvas'),qrLinkInput=document.getElementById('qrLinkInput'),qrCopyBtn=document.getElementById('qrCopyBtn'),analyticsModal=document.getElementById('analyticsModal'),shareAnalyticsBody=document.getElementById('shareAnalyticsBody');let currentShareTarget=null;
let currentXhr = null;
...
function renderShareRows(rows) {
  ...
  shareList.innerHTML = rows.map(row => {
    ...
    const title = row.shareTitle || currentShareTarget?.name || (currentShareTarget?.type === 'folder' ? 'Shared folder' : 'Shared file');
    const unread = Number(row.unreadNotifications || 0);
    let linkActions = '';
    if (active && row.canCopy) {
      linkActions += '<button type="button" class="share-history-copy js-copy-share" data-copy-mode="copy" data-share-record-id="' + Number(row.id) + '">Copy</button>';
      linkActions += '<button type="button" class="share-history-qr js-qr-share" data-share-record-id="' + Number(row.id) + '">QR</button>';
    } else if (active && row.canRotate) {
      linkActions += '<button type="button" class="share-history-copy rotate js-copy-share" data-copy-mode="rotate" data-share-record-id="' + Number(row.id) + '">Replace & copy</button>';
    }
    const analyticsAction = '<button type="button" class="share-analytics-button js-share-analytics" data-share-record-id="' + Number(row.id) + '">Activity' + (unread ? '<span class="share-notification-badge">' + unread + '</span>' : '') + '</button>';
    ...
    const sender = row.displayName ? ' · shared by ' + escapeHtml(row.displayName) : '';
    return '<div class="share-row-item"><div class="share-row-main"><div class="share-row-title">' + escapeHtml(title) + '</div><div class="share-row-line"><span class="share-status ' + escapeHtml(row.status) + '">' + escapeHtml(row.statusLabel) + '</span>' + escapeHtml(expires) + '</div><div class="share-row-meta">' + escapeHtml(limit) + ' · ' + Number(row.viewCount || 0) + ' views · created ' + escapeHtml(formatShareDate(row.createdAt)) + sender + '</div></div><div class="share-row-actions">' + linkActions + analyticsAction + revokeAction + '</div></div>';
  }).join('');
}
...
async function openShareModal(button) {
  ...
  shareHelp.textContent = folder
    ? 'Anyone with the link can browse this folder, preview files and subfolders, select downloads, or save the folder as a ZIP.'
    : 'Anyone with the link can preview or download only this file.';
  shareDuration.value = '7d';
  shareMaxDownloads.value = '0';
  sharePageTitle.value = currentShareTarget.name;
  shareDisplayName.value = vaultConfig.siteUsername || '';
  shareMessage.value = '';
  shareNotifyFirstOpen.checked = false;
  shareNotifyLimit.checked = false;
  shareNotifyExpiring.checked = false;
...
shareForm.addEventListener('submit', async event => {
  ...
  try {
    const body = {
      ...
      maxDownloads: Number(shareMaxDownloads.value || 0),
      shareTitle: sharePageTitle.value.trim(),
      displayName: shareDisplayName.value.trim(),
      shareMessage: shareMessage.value.trim(),
      notifyFirstOpen: shareNotifyFirstOpen.checked,
      notifyDownloadLimit: shareNotifyLimit.checked,
      notifyExpiring: shareNotifyExpiring.checked,
    };
...
function showQrCode(link) {
  ...
  if (typeof window.QRCode !== 'function') {
    qrCodeCanvas.innerHTML = '<div class="qr-code-error">The QR generator could not load. Copy the link instead.</div>';
  } else {
    new window.QRCode(qrCodeCanvas, {
      text: link,
      width: 204,
      height: 204,
      colorDark: '#050816',
...
shareQrBtn.addEventListener('click', () => showQrCode(shareLinkInput.value));
qrCopyBtn.addEventListener('click', async () => {
  const copied = await copyText(qrLinkInput.value);
  qrCopyBtn.textContent = copied ? 'Copied' : 'Select link';
  if (!copied) { qrLinkInput.focus(); qrLinkInput.select(); }
  setTimeout(() => qrCopyBtn.textContent = 'Copy link', 1800);
});

async function getStoredShareLink(shareId, button, rotate = false) {
  if (!shareId) throw new Error('Share link not found.');
  if (rotate && !confirm('This older link was created before repeat-copy support. Replace it with a new link? The old link will immediately stop working.')) {
    return null;
  }
  const endpoint = vaultConfig.urls.sharesBase + '/' + shareId + '/' + (rotate ? 'rotate' : 'link');
  return fetchJson(endpoint, rotate ? { method: 'POST', body: '{}' } : { method: 'GET' });
}

function analyticsLabel(type) {
  const labels = {
    share_view: 'Shared file opened', folder_view: 'Shared folder opened', file_open: 'File previewed',
    file_download: 'File downloaded', folder_download: 'Folder downloaded', selection_download: 'Selected files downloaded',
    notification_first_open: 'First open notification', notification_download_limit: 'Download limit reached', notification_expiring: 'Link expiring soon', folder_download_prepared: 'Folder download prepared', selection_download_prepared: 'Selection download prepared',
  };
  return labels[type] || String(type || 'Activity').replaceAll('_', ' ');
}

function analyticsIcon(type) {
  if (String(type).includes('download')) return 'DL';
  if (String(type).includes('view') || type === 'file_open') return 'OPEN';
  return 'INFO';
}
...
function renderAnalytics(data) {
  ...
  const downloads = Number(counts.file_download || 0) + Number(counts.folder_download || 0) + Number(counts.selection_download || 0);
  const opens = Number(counts.share_view || 0) + Number(counts.folder_view || 0) + Number(counts.file_open || 0);
  const prepared = Number(counts.folder_download_prepared || 0) + Number(counts.selection_download_prepared || 0);
  const topFiles = Array.isArray(analytics.topFiles) ? analytics.topFiles : [];
  const recent = Array.isArray(analytics.recent) ? analytics.recent : [];
  const topFileRows = topFiles.length ? '<div class="analytics-section-title">Most active files</div><div class="analytics-top-files">' + topFiles.map(file => '<div class="analytics-top-file"><strong>' + escapeHtml(file.file_name || 'File') + '</strong><span>' + Number(file.opens || 0) + ' opens · ' + Number(file.downloads || 0) + ' downloads</span></div>').join('') + '</div>' : '';
  const eventRows = recent.length ? recent.map(event => {
    const details = event.details || {};
    const detailText = details.filename || details.folder || details.path || details.message || '';
    return '<div class="analytics-event"><span class="analytics-event-icon">' + escapeHtml(analyticsIcon(event.event_type)) + '</span><div class="analytics-event-copy"><strong>' + escapeHtml(analyticsLabel(event.event_type)) + '</strong><span>' + escapeHtml(detailText || 'Public share activity') + '</span></div><time class="analytics-event-time">' + escapeHtml(formatShareDate(event.created_at)) + '</time></div>';
  }).join('') : '<div class="analytics-empty">No activity has been recorded for this link yet.</div>';
  shareAnalyticsBody.innerHTML = '<div class="share-analytics-summary">'
    + '<div class="analytics-stat"><strong>' + Number(analytics.uniqueVisitors || 0) + '</strong><span>Unique sessions</span></div>'
    + '<div class="analytics-stat"><strong>' + opens + '</strong><span>Opens / previews</span></div>'
    + '<div class="analytics-stat"><strong>' + downloads + '</strong><span>Completed downloads</span></div>'
    + '<div class="analytics-stat"><strong>' + prepared + '</strong><span>ZIP attempts</span></div></div>'
    + '<div class="analytics-dates"><div class="analytics-date">Last opened<strong>' + escapeHtml(analytics.lastOpenedAt ? formatShareDate(analytics.lastOpenedAt) : 'Not opened yet') + '</strong></div><div class="analytics-date">Last downloaded<strong>' + escapeHtml(analytics.lastDownloadedAt ? formatShareDate(analytics.lastDownloadedAt) : 'No completed downloads') + '</strong></div></div>'
    + topFileRows
    + '<div class="analytics-section-title">Recent activity</div><div class="analytics-event-list">' + eventRows + '</div>';
}
...
shareList.addEventListener('click', async event => {
  ...
  if (copyButton && !copyButton.disabled) {
    ...
    try {
      const data = await getStoredShareLink(shareId, copyButton, rotate);
      if (!data) { copyButton.disabled = false; copyButton.textContent = originalLabel; return; }
      ...
      if (!copied) { shareLinkInput.focus(); shareLinkInput.select(); }
      if (rotate) await loadShareRows();
      else setTimeout(() => { if (document.body.contains(copyButton)) { copyButton.disabled = false; copyButton.textContent = originalLabel; } }, 1800);
    } catch (error) {
  ...
  const qrButton = event.target.closest('.js-qr-share');
  if (qrButton && !qrButton.disabled) {
    qrButton.disabled = true;
    const original = qrButton.textContent;
    qrButton.textContent = 'Loading…';
    try {
      const data = await getStoredShareLink(Number(qrButton.dataset.shareRecordId || 0), qrButton, false);
      showQrCode(data?.shareUrl || '');
    } catch (error) {
      alert(error.message || 'Could not load the QR code.');
    } finally {
      qrButton.disabled = false;
      qrButton.textContent = original;
    }
    return;
  }

  const analyticsButton = event.target.closest('.js-share-analytics');
  if (analyticsButton && !analyticsButton.disabled) {
    analyticsButton.disabled = true;
    shareAnalyticsBody.innerHTML = '<div class="share-loading">Loading activity…</div>';
    openModal(analyticsModal);
    try {
      const data = await fetchJson(vaultConfig.urls.sharesBase + '/' + analyticsButton.dataset.shareRecordId + '/analytics', { method: 'GET' });
      renderAnalytics(data);
      await loadShareRows();
    } catch (error) {
      shareAnalyticsBody.innerHTML = '<div class="analytics-empty">' + escapeHtml(error.message || 'Could not load activity.') + '</div>';
    } finally {
